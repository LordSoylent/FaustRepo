...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("FAUMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("FAUMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("FAUMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("faust-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("FAUMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running FAUMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadFAUMiner() exception");
    } catch (...) {
        LogPrintf("ThreadFAUMiner() exception");
    }

    LogPrintf("ThreadFAUMiner exiting\n");
}
